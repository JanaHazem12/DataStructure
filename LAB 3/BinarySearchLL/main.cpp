...
class LinkedList{
    ...
    int BinarySearch(int target){
        ...
        int start=0;
        int middle=0;
        int end = counter-1; // counter starts from 0 , 1st node added counter = 1 now
        //middle=(start+end)/2;
        //int countToMiddle = 0;
        Node * MiddleNode;
        while(start<=end){ // start!=end+1 is okk BUT start<= read is better for readability
            ...
            middle=(start+end)/2;
            MiddleNode=getNodebyIndex(middle); // MiddleNode
            NumofIterations++;
            if(MiddleNode->data==target){
                cout<<"Num of Iterations: "<<NumofIterations<<endl;
                return middle;
            }
            else if(MiddleNode->data<target){
